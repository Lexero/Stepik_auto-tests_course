...
class TestMainPage1():
    ...
    @pytest.mark.smoke
    @pytest.mark.xfail(reason="fixing this bug right now") #С этим маркером тест выполнится, но если он не пройдет проверку, то прогон все равно будет пзеленым (можно использовать с известными багами, пока их чинят)
